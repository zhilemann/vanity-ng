...
static void ed_modmul(bn R, const bn X, const bn Y) {
	bn_2 T; bn_mul512(T, X, Y);

	// `2^256 ~= 38`
	u32 ext = bn_muladd(T, T, 38, T+8);
	bn_add32(T, T, 38 * ext);

	if (bn_cmp(T, ED25519_M) > LT)
		bn_sub(T, T, ED25519_M);
...
void ed_norm256(xyzt R[256], const xyzt P[256]) {
	...
	bn Z_inv[256]; ed_inv256(Z_inv, Zs, 4);
